...
//                .sorted(Comparator.reverseOrder()) // stream 안에 요소들을 내림차순(높은 값 → 낮은 값)으로 정렬 & Comparator.reverseOrder() 정렬을 반대로 하라는 코드
//                .collect(Collectors.toList()); // 정렬된 stream을 다시 리스트에 모아서 반환 > 최종 결과가 expectedSortedPrice 여기에 담긴다
//
//        Assertions.assertEquals(expectedSortedPrice, actualPrices, "상품 가격이 높은 순으로 정렬되지 않음.");
